    ...
    <div class="min-h-[calc(100vh-4rem)] flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">       
        <div class="w-full bg-white rounded-lg shadow md:mt-0 sm:max-w-md xl:p-0">           
            <div class="p-6 space-y-4 md:space-y-6 sm:p-8">               
                ...
                <form class="space-y-4 md:space-y-6" action="{{ route('post.update', $post->id) }}" method="POST" enctype="application/json">    
                    @csrf 
                    @method('PUT')    
                    <div>                       
                    ...
                    <div id="preview-container" class="{{ $post->image ? '' : 'hidden' }}">
                        <h3 class="block mb-2 text-sm font-medium text-gray-900">Preview</h3>
                        <div class="relative">
                            <img id="image-preview" src="{{ asset('storage/' . $post->image)}}" alt="Preview" class="w-full h-48 object-cover rounded-lg border">
